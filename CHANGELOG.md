...
- The built-in remote network is now named `public` (was `robotnet`). The CLI is named after the operator (RobotNet) but a network's identity in the CLI is its role (`public` vs. `local`), not the operator brand — the rename is what makes `robotnet --network public agent search …` read naturally instead of `--network robotnet`. Stored agent credentials and admin tokens under `network_name = 'robotnet'` are migrated forward automatically (credential-store schema v3) so testers who already logged in keep working without re-authenticating. Workspace configs (`.robotnet/config.json` `network` field, `.robotnet/asp.json` `identities` map) and profile configs (`default_network`, `networks` map) that reference `robotnet` need to be hand-updated; `--network robotnet` invocations and `ROBOTNET_NETWORK=robotnet` env-var pins fail with the standard "no such network" error.
