...
} from "../src/asmtp/auth-resolver.js";
...
describe("resolveAgentWebsocket", () => {
  it("returns the network's /connect URL plus the resolved bearer", async () => {
    const resolved = await resolveAgentWebsocket(makeConfig(), "@cli.bot", "tok");
    assert.equal(resolved.wsUrl, "ws://127.0.0.1:8723/connect");
